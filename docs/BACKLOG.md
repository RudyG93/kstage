...
- **Photos membres — restes (maj R8.1)** : la classe « split d'ère intra-groupe » est **résolue** (109 stale → 12 après le fix de résolution `refreshMemberPhotos` + passe `--stale`, 27 groupes mixtes → 0). Kim Chaewon/ARTMS OK. **Restent 12 SOLISTES** (Lisa, Jisoo, Taeyeon, Taemin, Solar, Soojin, YENA, Gaho, KANG DANIEL, LEE HI, Paul Kim, Crush) : leur page fandom est une désambiguïsation → le repli `generator=search` les exclut volontairement (risque d'homonyme, cf. Lisa/LiSA). Sous-classe distincte, sans split d'ère (une seule photo sur la page soliste). À traiter via une résolution désambiguïsée dédiée (parent connu) si besoin. Audit : `scripts/audit-member-photos.ts`.
