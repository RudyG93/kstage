...
>
> **Triage 2026-07-20** (balayage multi-agents du code réel vs BACKLOG) : items marqués « à faire » mais **déjà livrés** → clôturés (nav calendrier client, rails /mv, admin hub, `revalidatePath` doublé). Feature **suggestion communautaire** : l'UI publique de soumission avait été retirée au profit du widget Feedback → code mort (server actions + validateurs) **supprimé** ce jour ; modération admin + tables conservées. **Vraiment ouverts** (vérifiés) : toggle « Birthdays » mort (§notifs), matching des **titres** de MV qui n'exploite pas `name_aliases` (§round 10/17), warning silencieux `fetchSbsBoardLatestPost`.
...
- **3 toggles/migrations Supabase** (advisors vérifiés 2026-06-15) :
  ...
  - ✅ **leaked-password protection** → activée par Rudy (dashboard Supabase Auth), confirmée 2026-06-16 : l'advisor sécurité ne la liste plus. Advisors re-runés : restent `citext in public` + `group_follow_counts`/`profile_stats`/`consume_rate_limit` SECURITY DEFINER — **intentionnels/acceptés** (les fonctions agrègent des compteurs à travers des tables sous RLS et n'en renvoient aucune ligne privée ; `SECURITY INVOKER` casserait l'agrégation, révoquer casserait les pages publiques + le rate-limit ; anon/authenticated par design). **Soldé (2026-07-15, migration 0052)** : `ALTER EXTENSION citext SET SCHEMA extensions` (autorisé par Rudy) — le déplacement se fait par OID, `profiles.username` reste citext + case-insensitive (vérifié en prod), advisor `extension_in_public` disparu. Restent seulement les 3 fonctions SECURITY DEFINER (intentionnelles) + 2 RLS deny-all (INFO).
...
- ✅ **Hygiène code** — fait 2026-06-16 (code mort `getMemberMvs`, `grouped-event-list.tsx`, `ui/card.tsx`) **+ 2026-07-20** : `revalidatePath('/')` doublé dans `approveSuggestion` corrigé ; le « commentaire périmé `youtube-embed.tsx` » était en fait exact (vérifié) — item clos. Passe dead-code du 2026-07-20 : `countUserComments`/`deleteRating`/`SCORE_STEP`/`COMEBACK_SEND_UTC` retirés + feature suggestion publique démantelée (cf. bannière triage). ⚠️ `ui/toggle.tsx`/`ui/toggle-group.tsx` désormais orphelins tous les deux (vendored shadcn) — laissés en place (boilerplate ; à retirer si un jour on élague la lib UI).
...
- ~~**Calendrier — nav de mois full-client**~~ → **FAIT** (vérifié 2026-07-20, livré 2026-07-18) : changement de mois 100 % client (fetch `/api/calendar/month` CDN, cache mémoire par mois, prefetch adjacents, `history.replaceState`) — plus de nav RSC `<Link ?month>`.
...
- ~~**Rails sur le MV individuel** (`/mv/[slug]`)~~ → **FAIT** (vérifié 2026-07-20) : `/mv/[slug]` est enveloppé dans `<PageRails right={<MvRail…>}>` (rail catalogue du groupe, repli SidebarRight si vide).
...
- ~~Matching hangul-only~~ → **FAIT pour lineups/stages (2026-07-18, Lot 5)** : colonne `groups.name_aliases` (0061), 39 groupes seedés (hangul + membre facturé « 연준 »→TXT), branchée dans le matching lineups ET stage-links — Music Bank 1295 passé 12/15 → 15/15 stages. **⚠️ RESTE OUVERT (vérifié 2026-07-20)** : le matching des **titres de MVs YouTube** (`matchesGroup` dans `youtube.ts` + `channel-discovery.ts`) ne passe PAS `name_aliases` → un MV titré « NOWADAYS(나우어데이즈) » ne s'attribue pas à NOWZ, tout groupe hangul/aliasé pareil. Fix = passer `extraAliases` (= `name_aliases` du groupe) aux 2 call-sites, comme les crons music-shows le font déjà.
...
- ✅ **Purge des orphelins corrompus** (2026-08-20, accord Rudy) : 19 objets group-photos supprimés, scan final 0 problème sur les 3 buckets — incident webp clos, scripts tmp supprimés.
...
- ✅ **Lot 4 perf** (2026-08-20, merge `5cffbb3`) : /groups 550→234 Ko (−57 %, fenêtre 48 tuiles + listes slim), LCP priority 8 tuiles, /calendar 371→348 Ko (−6 % assumé : flight dominé par l'arbre rendu), data cache public + revalidateTag post-cron, sitemap +épisodes. `/calendar` en cache complet nécessiterait de convertir la chaîne getEventsForMonth au client anon — non fait, TTFB déjà bon.
...
- ✅ **Blocs de rail nourris par la base** (2026-08-22, merge `207bd85`) : `On music shows · 30 days` (/mvs, /groups), `Debut class of YYYY` (/groups/[slug], /artists/[slug]), `Rookies` (/calendar), à la place de « Recent discussions » qui ne rendait rien (6 commentaires, seuil 3). Idées écartées **après mesure** : « MV/Releases this week » (redit WeekGlance, la grille /calendar et le mur /mvs), « Most followed » (52 follows de 3 comptes ; `/groups` a déjà le tri `pop_desc`). ⚠️ Reste ouvert : ce tri `pop_desc` de /groups classe 260 groupes sur ces 3 comptes, et `subscriber_count` porte le nombre du **label** pour ~94 groupes sur 247.
- ✅ **Sorties annoncées sans titre** (2026-08-22, merge `7bbdf9e`) : `comeback-ingest` remplace un titre-descripteur (« NCT 127 7th Full Album ») par le vrai titre quand la source se corrige, + `scripts/refresh-untitled-releases.ts` pour le passé (6 corrigées en prod, `source_url` suivi).
...
- ~~**Admin hub** (`/admin`)~~ → **FAIT** (vérifié 2026-07-20) : `src/app/admin/page.tsx` = grille de 7 liens (Images/Events/Banners/Debuts/Suggestions/Feedback/Reports) + cards Source health + Activation.
- **Suggestions** (feature en pause depuis le retrait de l'UI publique, 2026-07-20) : si un jour on rouvre la contribution communautaire (au-delà du widget Feedback actuel), édition admin avant approbation + édition/suppression par l'auteur (policies `update`/`delete` own-rows sur `event_suggestions`) + notif push au contributeur (infra prête). Les server actions publiques ont été retirées ; la modération admin + les tables restent.
- **Music shows — collecte** : ✅ **bascule sur les chaînes des diffuseurs le 2026-08-21** (`docs/SCRAPING.md §3.25`) — épisodes, lineups réels, `stage_url` et numérotation reconstruits depuis les vidéos postées ; cron `aired-shows` 3×/jour ; nouvelle section **Stages** sur les pages groupe et artiste. Restent ouverts : les ~42 artistes de lineup hors roster (file `lineup_unmatched`, décision produit), les passages non confirmés en revue (`episodes_unconfirmed_lineup`), et Music Core 2026 non numéroté sur 2 épisodes (page Wikipedia contradictoire).
...
- ⚠️ **Community pulse** — **retiré depuis** (constaté 2026-07-20) : le modal 3-tabs `suggest-event-dialog.tsx` n'existe plus ; la contribution passe désormais par le seul **widget Feedback** (`feedback-dialog.tsx`). Les server actions de soumission publique ont été supprimées ce jour (code mort). Réactivation = cf. item « Suggestions » ci-dessus.
