...
- **E2E en CI** : le job existe (`ci.yml`, gated `vars.E2E_ENABLED`) — reste l'action GitHub UI (variable + 4 secrets) pour l'activer. Une fois posée, vérifier que le job `e2e` apparaît ET passe sur la prochaine PR.
...
- 🐛 **Doublons music_show en DB (cause racine)** — découvert 2026-07-05 : l'enrichissement stage-links UPDATE `source_url`, qui fait partie de la clé d'idempotence `(group_id, type, start_at, source_url)` → le scrape carrd suivant réinsère une row doublon (14 paires en prod). La dédup display-level est livrée (`groupMusicShowEpisodes`), mais le vrai fix = découpler : colonne dédiée `stage_url` pour l'enrichissement (source_url reste la clé), OU dédup scraper par (group_id, type, start_at) avant insert. + purge one-shot des 14 paires existantes (garder la row enrichie). À faire au prochain passage scraper.
