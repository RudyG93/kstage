...
- [x] **Limite uploads-playlist incomplète** (§2) : ✅ fait 2026-07-17 (Phase 3 Lot 4) — `scripts/backfill-mv-search.ts <group-slug> <channelId>` : `search.list` ciblé chaîne+nom (100 units), hits **injectés** dans `scrapeGroup` via `opts.uploads` (gates titre/durée, dédup ±14 j, slug, mv_kind = le pipeline du cron, zéro duplication). **One-shot manuel uniquement** (jamais en cron : 100 units × ~90 sources = intenable). Prérequis : la row `sources` du groupe avec `channel_id` posée à la main (chaîne partagée type 1theK ≠ chaîne du groupe = décision humaine). Cas connu : SUCTION/1theK (playlist uploads > 20k = impaginable, cap API).
