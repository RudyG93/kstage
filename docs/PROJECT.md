...
> **Programme actif** : roadmap audit-driven 0→5 (détail daté dans `docs/JOURNAL.md`). **Phase 0 (vérité produit) faite** ; **Phase 1 (contrat de confiance) CLOSE** (2026-07-16) — Lots 1 (`hidden` partout), 2 (dates `tentative`), 3a + 3a-polish (timezone par viewer, toutes surfaces), 3b (push par fuseau d'abonné), 4 (budget notifs : `announced` coupé, digest idempotent via `digest_log`, échecs comptés), 5 (monitoring actif : cron `monitor` + carte « Source health » /admin). **Prochaine étape : Phase 2** (activation/UX/analytics, north-star = actifs hebdo qui ouvrent leur calendrier).
