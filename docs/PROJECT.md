...
> **Programme actif** : roadmap audit-driven 0→5 (détail daté dans `docs/JOURNAL.md`). **Phases 0 et 1 CLOSES** (2026-07-16). **Phase 2 (activation/UX/analytics) : Lots 1-4 livrés** — analytics first-party `product_events` + dashboard « Activation » /admin + baseline Lighthouse, funnel d'activation orienté calendrier (landing réordonnée, onboarding cherchable → « See your calendar », email → /calendar), UX mobile + labels honnêtes (« In the spotlight », Top rated seuil 2 votes + count), passe a11y (skip-link, headings, focus, ticker SR, contrastes clairs ≥ 4.5:1). **Lot 5 (consolidation typo ~20→10 niveaux) : branche `feat/typo-light` poussée, EN ATTENTE de review visuelle Rudy (preview Vercel) avant merge.** North-star mesurée : `calendar_opened` (home connectée OU /calendar, dédup 1/jour).
