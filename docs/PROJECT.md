...
> **Programme actif** : roadmap audit-driven 0→5 (détail daté dans `docs/JOURNAL.md`). **Phases 0, 1 et 2 CLOSES** (2026-07-16). Phase 2 = 5 lots : analytics first-party `product_events` + dashboard « Activation » /admin + baseline Lighthouse · funnel d'activation orienté calendrier (landing CTA 2ᵉ écran, onboarding cherchable → « See your calendar », email → /calendar) · UX mobile + labels honnêtes (« In the spotlight », Top rated seuil 2 votes + count) · passe a11y · consolidation typo ~20→10 niveaux (review Rudy). North-star mesurée : `calendar_opened` (home connectée OU /calendar, dédup 1/jour). **Prochaine étape : Phase 3** (tiers Vérifié/Surveillé/Candidat + reproductibilité DB §9.2) ; la Phase 4 (bêta 20-50 testeurs) dépend du recrutement par Rudy.
