...
> **Round « Infrastructure & Data » 2026-07-18** (11 lots + fix CI, détail daté dans `docs/JOURNAL.md`) : fonctions Vercel **colocalisées Supabase** (`cdg1` ↔ `eu-west-3`, `getViewer()` dédup auth, Speed Insights) · pipeline images **normalisé sharp** (856 objets retraités, ~2 Go — la photo SuA pesait 17,2 Mo) + photos immédiates à la création · **data-health monitor** (`/admin/health`, 12 checks des classes d'erreur connues + résumé quotidien scrape_log) · dédup de personnes cross-groupe (`canonical_id` auto à l'ingest, SuA/JiU/Yoohyeon UAU→Dreamcatcher) · **aliases hangul/membre facturé** (0061 — MB 1295 = 15/15 stages) · **numérotation d'épisodes par autorité Wikipedia** (le « 1295 » du 17/07 = épisode **1299** ; +82 stages passés backfillés) · **roster watch** des (pre)debuts (OURBIRTHDAY complété tout seul) + upgrade de titres placeholder · catalogue MV (sweep + discovery ×10/lundi + deep re-scan rotatif 0062) · **calendrier : mois 100 % client** (`/api/calendar/month` CDN) + content-visibility · fil commentaires **v4 Reddit** · /mv 3 colonnes + back fallback + token `--hover`. Migrations 0061 + 0062 appliquées.

> **Trois rounds de retours Rudy le 2026-07-17** (détail daté dans `docs/JOURNAL.md`) : nuit de vérification (timezone anniversaires = dates civiles, favicon.ico, lisibilité push) + round #1 (aggregator future-only → Inkigayo 1318, histogramme demi-points, rail commentaires, tabs Groups/Solo client, notifs event-led post-scan, sweep MVs) + round #2 (picker complet, crash /admin/debuts, dédup solistes recherche, **romanisation des titres MV**, pipeline `lineup_unmatched`, **enrichissement MusicBrainz**, **page épisode `/show/[show]/[day]`**). Migrations 0059 (lineup_unmatched) + 0060 (show_episodes + comments.episode_id) appliquées.

> **Programme actif** : roadmap audit-driven 0→5 (détail daté dans `docs/JOURNAL.md`). **Phases 0, 1, 2 et 3 CLOSES** (2026-07-17). Phase 3 « couverture large autonome » = 4 lots : base fraîche reproductible (seed réparé, sources structurelles versionnées, **job CI DB** `supabase db reset` path-filtré) · tiers de confiance **internes** `groups.confidence` (verified/monitored/candidate — candidate = noindex + hors sitemap + jamais notifié ; 166 verified / 5 monitored / 0 candidate au backfill) · pipeline candidat→publication (cron hebdo `discover-channels`, création récupérable, `debutGateDecision`, **gate de sortie testé dans les deux sens**) · robustesse scraping (provenance réelle des fallbacks 0058, fusion tentative→confirmed, Jina timeout+retry+clé optionnelle, backfill chaînes impaginables). **Prochaine étape : Phase 4** (bêta fermée 20-50 testeurs — dépend du recrutement par Rudy ; le dashboard « Activation » /admin est prêt à mesurer les seuils §10.4), puis Phase 5 (SEO/juridique/premium après rétention prouvée).
...
**Chiffres prod (2026-07-18)** : **192 groupes** (156 groupes + 36 solistes), **2 471 MVs visibles**, **33 épisodes** de music-show numérotés par autorité Wikipedia, **1 011 membres dont 96 % avec photo** (normalisées ≤800px webp), **727 tests unitaires (758 avec contrats CI) + 28 E2E**, bannières YT 2560px, **722 tests unitaires + 28 E2E** (✅ job e2e GitHub **activé + vert** depuis le 2026-07-15 : variables montées au niveau repo + `ci.yml` lit `vars.*` ; run `78266ee` success. Reste conseillé : un compte test dédié — `E2E_AUTH_EMAIL` = compte perso de Rudy). **Tier premium neutralisé** (2026-07-14) : surface inerte retirée de l'UI, socle DB `profiles.tier` conservé pour un futur round Stripe. **Socle scraping activé** (R10.1) : crons GitHub Actions opérationnels (music-shows 2×/j, youtube, comebacks, images). App : https://kstage.vercel.app/

**Pages live** : `/` (landing / home 8 modules), `/calendar`, `/mvs`, `/mv/[slug]` (ratings + comments), `/show/[show]/[day]` (**épisode music-show** : logo, n°, lineup, stages YouTube, discussion), `/groups` (+tab solo, switch 100 % client), `/groups/[slug]`, `/artists/[slug]`, `/search`, `/u/[username]`, `/account` (notifs par type + calendar feed), `/admin/*`, `/api/ical/[token]`.
