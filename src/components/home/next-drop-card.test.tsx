...
describe('NextDropCard', () => {
  it('renders nothing when there is no upcoming event', () => {
    const { container } = render(<NextDropCard timeZone="Asia/Seoul" event={null} />)
    expect(container.firstChild).toBeNull()
  })

  it('renders the title and a 4-cell countdown (D / H / M / S)', () => {
    render(<NextDropCard timeZone="Asia/Seoul" event={makeEvent()} isAuthed />)
  ...
  it('shows a truthful follow CTA when the event has a group', () => {
    render(<NextDropCard timeZone="Asia/Seoul" event={makeEvent()} isAuthed isFollowing={false} />)
  ...
  it('shows a pressed Following button instead of a notification-settings link', () => {
    render(<NextDropCard timeZone="Asia/Seoul" event={makeEvent()} isAuthed isFollowing />)
  ...
  it('falls back to "?" when the group is missing', () => {
    render(
      <NextDropCard timeZone="Asia/Seoul" event={makeEvent({ groups: null, group_id: null })} />,
    )
